...
class DenseAndCounter(Module):
  ...
  @autonames
  def __call__(self, x):
    # Turn this module into a pure function
    def _call(scope, x):
      counter = Counter(scope)
      return Dense(scope, features=2)(counter(x))
    return lift.vmap(_call, split_rngs={True: False},
        variable_in_axes={'param': None, 'counter': 0},
        variable_out_axes={'param': None, 'counter': 0})(self.scope, x)
